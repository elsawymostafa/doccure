...
if ($_SERVER['REQUEST_METHOD']=='POST'){
	...
	$q="SELECT password from doctors WHERE id=$id";
	$pass;
	$rusalt=$conn->query($q);
	foreach ($rusalt as $r)
	   { $pass=$r['password'];}
	if (md5($oldpass)==$pass&&$newpass==$confpass)
	{
		$_SESSION['change']=1;
		$npass=md5($newpass);
		$q="UPDATE doctors SET password ='$npass' WHERE id =$id";
		$conn->query($q);
	}
	else
	$_SESSION['change']=2;
										...
										<div class="col-md-12 col-lg-6">
											...
											?>	
												<div class="alert alert-danger" role="alert">
													Wrong old password or dismitching password
												</div>
											<?php
											}
											?>
